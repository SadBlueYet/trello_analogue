...
async def create_comment(db: AsyncSession, comment_in: CommentCreate, user_id: int) -> Comment:
    ...
    comment = Comment(text=comment_in.text, card_id=comment_in.card_id, user_id=user_id)
    db.add(comment)
    await db.commit()
...
async def update_comment(db: AsyncSession, comment: Comment, comment_in: CommentUpdate) -> Comment:
    """Update a comment."""
