...
async def get_boards(db: AsyncSession, user_id: int) -> Sequence[Board]:
    result = await db.execute(select(Board).where(Board.owner_id == user_id).options(selectinload(Board.lists)))
    return result.scalars().all()


async def get_board(db: AsyncSession, board_id: int) -> Optional[Board]:
    result = await db.execute(select(Board).where(Board.id == board_id).options(selectinload(Board.lists)))
    return result.scalars().first()


async def create_board(db: AsyncSession, board_in: BoardCreate, user_id: int) -> Board:
    db_board = Board(
...
async def update_board(db: AsyncSession, db_board: Board, board_in: BoardUpdate) -> Board:
